...
public class FraudScoreEvaluatorService {
	...
	public ResponseInfo evaluateFraudScore(Double fraudScore) {
		...
		if(fraudScore > 100) {
			...
			log.info("Fraud Evaluated!!. Fraud Score[ " + fraudScore +"], Stauts["+highFraud.getScoreLevel()+"], Message["+highFraud.getName()+"]");
			return responseInfo;
		}
		...
		for(FraudScoreLevel fraudScoreLevel : FraudScoreLevelList) {
			if(fraudScore >= fraudScoreLevel.getMinimunRange() && fraudScore <= fraudScoreLevel.getMaximumRange()) {
				log.info("Fraud Evaluated!!. Fraud Score[ " + fraudScore +"], Stauts["+fraudScoreLevel.getScoreLevel()+"], Message["+fraudScoreLevel.getName()+"]");
