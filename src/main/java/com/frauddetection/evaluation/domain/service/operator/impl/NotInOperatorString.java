package com.frauddetection.evaluation.domain.service.operator.impl;

import java.text.ParseException;
import java.util.Arrays;
import java.util.List;

import com.frauddetection.shared.exception.InvalidInfoException;
import com.frauddetection.evaluation.domain.service.operator.OperatorDatatype;
import org.springframework.stereotype.Component;

@Component
public class NotInOperatorString implements OperatorDatatype {

    @Override
    public boolean applyOperation(String operand1, String operand2) throws ParseException, InvalidInfoException {
        if (operand1 == null || operand2 == null) {
            throw new IllegalArgumentException("Operands cannot be null for NOT_IN comparison");
        }

        List<String> operands = Arrays.asList(operand2.split(","));

        if (operands.isEmpty()) {
            throw new IllegalArgumentException(
                    String.format("operand2 must be a valid CSV list for NOT_IN comparison. operand2='%s'", operand2));
        }

        return !operands.contains(operand1);
    }
}
