...
import com.frauddetection.entitypreprocessing.EntityPreprocessor;
...
public enum EntityPreprocessorFactory {
	...
	AMOUNT(1) {
		...
		public EntityPreprocessor getEntityPreprocessor() {
			log.info("Returing EntityPreprocessor: AMOUNT");
			return entityPreprocessor;
		}
	},DEFAULT(0) {
		@Override
		public EntityPreprocessor getEntityPreprocessor() {
			log.info("Returing EntityPreprocessor: DEFAULT");
			return entityPreprocessor;
		}
	};
	...
	public static EntityPreprocessorFactory getEntityPreprocessorEnum(long entityId) {
		Optional<EntityPreprocessorFactory> entity = Arrays.stream(EntityPreprocessorFactory.values())
				.filter(c -> c.getApiId() == entityId).findFirst();
		if(!entity.isPresent()) {
