...
import com.frauddetection.rules.operational.service.operatordatype.OperatorDatatype;
...
public enum OperatorDatatypeFactory {
	...
	public static OperatorDatatypeFactory getOperatorDataTypeEnum(long operatorId, long dataTypeId) {
		Optional<OperatorDatatypeFactory> operator = Arrays.stream(OperatorDatatypeFactory.values())
				.filter(o -> o.getDataTypeId() == dataTypeId).filter(d -> d.getOperatorId() == operatorId).findFirst();
		return operator.get();
	}
